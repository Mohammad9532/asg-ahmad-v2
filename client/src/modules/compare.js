...
function renderComparisonTable(data, container, shop, focusCycleId) {
    ...
    columns.forEach(col => {
        const classes = col.isFocus
            ? "px-6 py-3 text-right text-[11px] font-bold text-teal-700 uppercase tracking-tight bg-teal-50 border-b-2 border-teal-200"
            : "px-6 py-3 text-right text-[11px] font-bold text-slate-400 uppercase tracking-tight border-b border-slate-100";
    ...
    rows.forEach((row) => {
        ...
        row.cells.forEach(cell => {
            ...
            cellHtml += `
                <td class="px-6 py-2.5 whitespace-nowrap text-[13px] ${styleClass} text-right transition-all duration-200 group relative block md:table-cell border-b border-slate-50 md:border-none flex justify-between items-center md:block">
                     <span class="md:hidden font-black text-slate-400 uppercase text-[10px] tracking-tight">${col.label}</span>
                    <span>${displayVal}</span>
                    <div class="absolute bottom-1 right-2 text-[9px] text-slate-300 opacity-0 group-hover:opacity-100 pointer-events-none hidden md:block">${cell.monthLabel}</div>
                </td>`;
        });
        ...
        rowsHtml += `
            <tr class="hover:bg-slate-50/80 transition-all duration-200 group border-b border-slate-50 last:border-0 block md:table-row border-b-4 border-slate-50 md:border-none mb-4 md:mb-0 relative bg-white">
                <td class="px-6 py-2.5 whitespace-nowrap text-[13px] font-bold text-slate-700 group-hover:text-teal-700 transition-colors block md:table-cell border-b border-slate-50 md:border-none bg-slate-50/50 md:bg-transparent flex justify-between items-center md:block">
                    <span class="md:hidden font-black text-slate-400 uppercase text-[10px] tracking-tight">Period</span> <span>${rowLabel}</span>
                </td>
                ${cellHtml}
                 <td class="px-6 py-2 md:py-4 whitespace-nowrap text-sm text-right relative block md:table-cell border-b border-slate-50 md:border-none flex justify-between items-center md:block">
                     <span class="md:hidden font-bold text-slate-500 uppercase text-xs">Target</span>
                     <input 
                        type="number" 
                        value="${inputVal}"
                        placeholder="${isAuto ? formatCurrency(targetVal).replace('AED ', '') : 'Set'}"
                        onchange="saveTarget('${shop}', ${gYear}, ${gMonthIndex}, this.value)"
                        class="block w-28 md:w-32 ml-auto text-right px-3 py-1.5 border border-slate-200 focus:border-teal-500 focus:ring-2 focus:ring-teal-200 rounded-lg text-slate-900 font-medium text-sm transition-all shadow-sm group-hover:shadow-md ${isAuto ? 'placeholder-slate-300' : 'bg-white'}"
                    >
                </td>
                <td class="px-6 py-4 whitespace-nowrap block md:table-cell md:border-none flex justify-between items-center md:block">
                    <span class="md:hidden font-bold text-slate-500 uppercase text-xs">Growth</span>
                    <div class="flex items-center gap-3 w-full md:w-40 justify-end md:justify-start">
                        <div class="flex-1 h-2 bg-slate-100 rounded-full overflow-hidden shadow-inner hidden md:block">
    ...
    columns.forEach(col => {
        ...
        totalCellsHtml += `<td class="px-6 py-2 md:py-5 whitespace-nowrap text-base font-bold ${styleClass} text-right block md:table-cell border-b border-slate-700 md:border-none flex justify-between items-center md:block">
            <span class="md:hidden font-bold text-slate-400 uppercase text-xs">${col.label} Total</span>
            <span>${formatCurrency(colTotals[col.id].actual)}</span>
        </td>`;
    });
    ...
    const totalsRow = `
        <tr class="bg-slate-900 border-t-[10px] border-white shadow-2xl relative z-10 text-white block md:table-row">
            <td class="px-6 py-5 whitespace-nowrap text-sm font-black text-slate-100 uppercase tracking-widest pl-8 block md:table-cell border-b border-slate-700 md:border-none flex justify-between items-center md:block">
                <span>Total</span>
            </td>
            ${totalCellsHtml}
            <td class="px-6 py-2 md:py-5 whitespace-nowrap text-base font-black text-white text-right tracking-wide block md:table-cell border-b border-slate-700 md:border-none flex justify-between items-center md:block">
                <span class="md:hidden font-bold text-slate-400 uppercase text-xs">Target Total</span>
                <span>${formatCurrency(focusTarget)}</span>
            </td>
            <td class="px-6 py-5 whitespace-nowrap block md:table-cell md:border-none flex justify-between items-center md:block">
                <span class="md:hidden font-bold text-slate-400 uppercase text-xs">Growth Total</span>
                <div class="flex items-center gap-3 w-40 justify-end md:justify-start">
                    <div class="flex-1 h-2 bg-slate-700 rounded-full overflow-hidden shadow-inner ring-1 ring-slate-600 hidden md:block">
    ...
    const html = `
        <div class="bg-white rounded-2xl shadow-xl border border-slate-200 overflow-hidden">
            <div class="">
                <table class="min-w-full tabular-nums border-collapse block md:table">
