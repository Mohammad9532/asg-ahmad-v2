...
export function switchEntryType(type) {
    ...
    // Auto-fetch latest bill no for bookings
    if (type === 'booking') {
        const shopSelect = document.getElementById('entryShop');
        if (shopSelect && shopSelect.value && shopSelect.value !== 'overview' && shopSelect.value !== 'compare' && shopSelect.value !== 'customers') {
            fetch(`${BASE_URL}/api/${shopSelect.value}/bookings/latest-bill`, {
                headers: { 'Authorization': `Bearer ${localStorage.getItem('authToken')}` }
            })
            .then(res => res.json())
            .then(data => {
                if (data && data.next) {
                    const billNoInput = container.querySelector('input[name="billNo"]');
                    if (billNoInput && !billNoInput.value) {
                        billNoInput.value = data.next;
                    }
                }
            })
            .catch(err => console.error("Error fetching latest bill:", err));
        }
    }
}
...
export async function handleAddEntrySubmit(event) {
    ...
    try {
        ...
        if (preview) {
            ...
            if (currentEntryType === 'booking') {
                const phoneWithCode = (payload.countryCode || '+971') + (payload.phone || '');
                const msg = `Hello ${payload.name}, your booking with Bill #${payload.billNo} for ${amountFormatted} has been saved at ${state.activeShop}. Thank you!`;

                summary = `
                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                        <div>
                            <b>Booking Saved:</b> Bill #${payload.billNo} - ${payload.name} (${amountFormatted})
                        </div>
                        <button onclick="sendWhatsApp('${phoneWithCode}', '${msg.replace(/'/g, "\\'")}')" 
                                class="flex items-center px-3 py-1.5 bg-green-500 hover:bg-green-600 text-white text-xs font-bold rounded-lg transition-colors shadow-sm">
            ...
            preview.innerHTML = `<div>${summary}</div>`;
            preview.classList.remove('hidden');
        }
